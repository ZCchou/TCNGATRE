...
def resolve_config(argv=None, description="Run H3_Large hparam variant.") -> TCNGATREConfig:
    args = _parse_dataset(argv, description)
    os.environ.setdefault(
        "UAV_TCNGATRE_RUN_ROOT",
        str(MODEL_ROOT / "runs" / f"{RUN_PREFIX}_{args.dataset}"),
    )
    # H3_Large: increased model capacity; batch_size halved to avoid OOM (d_model↑ 2× → ~4× attention memory)
    os.environ.setdefault("UAV_TCNGATRE_D_MODEL", "128")
    os.environ.setdefault("UAV_TCNGATRE_TCN_LAYERS", "6")
    os.environ.setdefault("UAV_TCNGATRE_TCN_BLOCKS", "5")
    os.environ.setdefault("UAV_TCNGATRE_LR", "3e-4")
    os.environ.setdefault("UAV_TCNGATRE_BATCH_SIZE", "64")
