...
RUN_PREFIX = "tcngatre_h4_longwindow"
...
def resolve_config(argv=None, description="Run H4_LongWindow hparam variant.") -> TCNGATREConfig:
    args = _parse_dataset(argv, description)
    os.environ.setdefault(
        "UAV_TCNGATRE_RUN_ROOT",
        str(MODEL_ROOT / "runs" / f"{RUN_PREFIX}_{args.dataset}"),
    )
    # H4_LongWindow: longer temporal context window; batch_size halved to offset 2× activation memory
    os.environ.setdefault("UAV_TCNGATRE_LOOKBACK", "256")
    os.environ.setdefault("UAV_TCNGATRE_HORIZON_OUT", "8")
    os.environ.setdefault("UAV_TCNGATRE_SAMPLE_STRIDE", "8")
