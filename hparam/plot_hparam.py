...
import numpy as np
import pandas as pd
import summarize_hparam as summarize_results
...
MODEL_DISPLAY = {
    ...
    "H2_Small": "H2 Small",
    "H3_Large": "H3 Large",
    "H4_LongWindow": "H4 LongWin",
    "H5_DenseGraph": "H5 DenseG",
    "H6_Conservative": "H6 Conserv",
}
MODEL_COLORS = {
    "H1_Default": "#3A86FF",
    "H2_Small": "#FF6B6B",
    "H3_Large": "#FFA500",
    "H4_LongWindow": "#2D6A4F",
    "H5_DenseGraph": "#7B2CBF",
    "H6_Conservative": "#E63946",
}
RUN_PREFIX = {
...
def parse_args(argv: list[str] | None = None):
    parser = argparse.ArgumentParser(
        description="Plot grouped bar charts from TCNGATRE hparam metric summaries."
    )
    parser.add_argument("--models", nargs="+", default=MODEL_ORDER)
    parser.add_argument(
        "--stat-dir",
        default="statistic",
        help="Directory containing hparam_metrics_*.csv. Relative paths resolve from hparam root.",
    )
    parser.add_argument(
        "--out-dir",
...
def relative_to_hparam(path: Path) -> str:
    try:
...
def status_summary_lines(stat_dir: Path) -> list[str]:
    status_path = stat_dir / "hparam_metrics_status.csv"
...
def safe_write_text(path: Path, text: str) -> bool:
    try:
        ...
        print(f"[WARN] could not write {relative_to_hparam(path)}: {type(exc).__name__}: {exc}")
        return False
...
def draw_grouped_metric_bars(
    ...
    width = min(0.15, 0.72 / max(len(model_keys), 1))
    ...
    ax.set_title(
        f"TCNGATRE Hparam | {title_average} | dataset={dataset} | threshold={title_method} | label={title_label}",
        fontsize=12,
        fontweight="bold",
        pad=14,
    )
    ...
    try:
        ...
        print(f"[WARN] could not write {relative_to_hparam(output_path)}: {type(exc).__name__}: {exc}")
        plt.close(fig)
        return None
    plt.close(fig)
    return PlotSpec(
...
def run_root_for(model_key: str, dataset: str) -> Path:
    ...
    return HPARAM_ROOT / model_key / "runs" / f"{prefix}_{dataset}"
...
def draw_threshold_timeseries(
    ...
    df = flight_df.sort_values([x_col], kind="mergesort").reset_index(drop=True)
    ...
    ax.text(0.0, -0.15,
            "Red background = ground-truth anomaly.",
...
def plot_all(args) -> tuple[list[PlotSpec], list[ThresholdPlotSpec]]:
    ...
    csv_by_average = {
        "macro": stat_dir / "hparam_metrics_macro.csv",
        "micro": stat_dir / "hparam_metrics_micro.csv",
    }
    data_by_average = {
...
def write_plot_index(
    ...
    for average, path in csv_by_average.items():
        ...
        lines.append(f"- `{average}`: `{relative_to_hparam(path)}` (`{rows}` rows)")
    lines.extend(["", "## Generated Metric PNGs", ""])
    if not created:
        lines.append("No metric plots generated. Run hparam pipeline first.")
    else:
        for spec in created:
            rel_path = relative_to_hparam(spec.output_path)
            lines.append(
...
def main(argv: list[str] | None = None) -> int:
    ...
    out_dir = resolve_path(args.out_dir)
    stat_dir = resolve_path(args.stat_dir)
    print(f"[OK] plot_dir={relative_to_hparam(out_dir)}")
    ...
    for spec in created:
        print(
            ...
            f"{spec.label_col} rows={spec.rows} path={relative_to_hparam(spec.output_path)}"
        )
    status_lines = status_summary_lines(stat_dir)
    if status_lines:
        print(f"[STATUS] {'; '.join(status_lines)}")
    print(f"[OK] index={relative_to_hparam(out_dir / 'plot_index.md')}")
    return 0
