...
def resolve_config(argv=None, description="Run H2_Small hparam variant.") -> TCNGATREConfig:
    args = _parse_dataset(argv, description)
    os.environ.setdefault(
        "UAV_TCNGATRE_RUN_ROOT",
        str(MODEL_ROOT / "runs" / f"{RUN_PREFIX}_{args.dataset}"),
    )
    # H2_Small: reduced model capacity for fast verification
    os.environ.setdefault("UAV_TCNGATRE_D_MODEL", "32")
    os.environ.setdefault("UAV_TCNGATRE_TCN_LAYERS", "3")
    os.environ.setdefault("UAV_TCNGATRE_TCN_BLOCKS", "3")
    os.environ.setdefault("UAV_TCNGATRE_LR", "1e-3")
    os.environ.setdefault("UAV_TCNGATRE_DROPOUT", "0.10")
