...
RUN_PREFIX = "tcngatre_h5_densegraph"
...
def resolve_config(argv=None, description="Run H5_DenseGraph hparam variant.") -> TCNGATREConfig:
    args = _parse_dataset(argv, description)
    os.environ.setdefault(
        "UAV_TCNGATRE_RUN_ROOT",
        str(MODEL_ROOT / "runs" / f"{RUN_PREFIX}_{args.dataset}"),
    )
    # H5_DenseGraph: more frequent and deeper graph correction
    os.environ.setdefault("UAV_TCNGATRE_INTERLEAVE_EVERY", "1")
    os.environ.setdefault("UAV_TCNGATRE_GRAPH_NUM_HOPS", "3")
    os.environ.setdefault("UAV_TCNGATRE_GRAPH_BETA", "0.6")
