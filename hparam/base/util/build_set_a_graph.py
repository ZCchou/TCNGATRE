# -*- coding: utf-8 -*-
"""
Build a MIC-based graph for set_a-style model nodes.
Copied from TCNGATRE/util/build_set_a_graph.py for ablation isolation.
"""
...
ALFANEW_FEATS = [
    "MNI/roll", "MNI/pitch", "MNI/yaw",
    "MVH/groundspeed", "MVH/heading", "MVH/throttle", "MVH/climb", "MVH/altitude",
    "MLP/linear_velocity_x", "MLP/linear_velocity_y", "MLP/linear_velocity_z",
    "MLP/angular_velocity_x", "MLP/angular_velocity_y", "MLP/angular_velocity_z",
]

SIMULATE_FEATS = [
    "u_cmd", "position", "velocity", "accel", "torque", "current", "voltage",
]
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--in_dir", default="dataset/alfa/wide_flights_set_a_causal/No_Failure")
    ap.add_argument("--out_dir", default="out_static_graph_set_a_causal_mic")
    ap.add_argument("--feature_suffix", default="_med_mm")
    ap.add_argument("--dataset_mode", default="set_a_causal",
                    choices=["set_a_causal", "alfanew", "simulate", "gpsdata"])
    ap.add_argument("--grid_sec", type=float, default=0.1)
    ap.add_argument("--mic_alpha", type=float, default=0.6)
    ap.add_argument("--mic_c", type=int, default=15)
    ap.add_argument("--min_overlap", type=int, default=128)
    ap.add_argument("--mic_threshold", type=float, default=0.0)
    ap.add_argument("--top_k_per_node", type=int, default=0)
    ap.add_argument("--max_points_per_pair", type=int, default=200000)
    ap.add_argument("--include_flights", nargs="*", default=None)
...
def maybe_subsample_pair(x: np.ndarray, y: np.ndarray, max_points: int) -> Tuple[np.ndarray, np.ndarray]:
    if len(x) <= max_points:
        return x, y
...
def compute_one_mic_pair(
    ...
    (i, j, src, dst, x, y, alpha, c, min_overlap, mic_threshold, max_points_per_pair) = payload
    ...
    if overlap >= int(min_overlap):
        x_pair = x[valid]
        y_pair = y[valid]
        x_pair, y_pair = maybe_subsample_pair(x_pair, y_pair, max_points=int(max_points_per_pair))
        mine = MINE(alpha=float(alpha), c=int(c))
        mine.compute_score(x_pair, y_pair)
        mic = float(mine.mic())
        kept = mic >= float(mic_threshold)
    return {"i": int(i), "j": int(j), "src": str(src), "dst": str(dst),
            "mic": float(mic), "overlap": int(overlap), "kept": int(kept)}
...
def progress_iter(items, total: int, desc: str, progress_every: int):
    ...
    for idx, item in enumerate(items, start=1):
        if idx == 1 or idx == total or idx % every == 0:
            ...
            print(f"[GRAPH] {desc}: {idx}/{total} pairs done ({idx/elapsed:.2f} pair/s)", flush=True)
        yield item
...
def compute_mic_adjacency(
    ...
    pair_payloads = [
        (i, j, src, dst, values[src], values[dst],
         float(alpha), int(c), int(min_overlap), float(mic_threshold), int(max_points_per_pair))
    ...
    if workers > 1:
        ...
        with ProcessPoolExecutor(max_workers=workers) as executor:
            ...
            edge_rows = list(progress_iter(iterator, total=total_pairs, desc="MIC pairs", progress_every=progress_every))
    else:
        print(f"[GRAPH] Fitting MIC serially over {total_pairs} pairs.", flush=True)
        edge_rows = [
            compute_one_mic_pair(payload)
            for payload in progress_iter(pair_payloads, total=total_pairs, desc="MIC pairs", progress_every=progress_every)
        ]
...
def main() -> int:
    ...
    print(f"[GRAPH] Loaded {len(file_rows)} files, rows={len(merged)}, "
          f"features={len(feat_cols)}, grid_sec={float(args.grid_sec):g}.", flush=True)
    adjacency, edge_rows = compute_mic_adjacency(
        df=merged, feat_cols=feat_cols,
        alpha=float(args.mic_alpha), c=int(args.mic_c),
        min_overlap=int(args.min_overlap), mic_threshold=float(args.mic_threshold),
        max_points_per_pair=int(args.max_points_per_pair),
        num_workers=int(args.num_workers), progress_every=int(args.progress_every),
    )
    threshold_kept_edges = int(sum(int(row["kept"]) for row in edge_rows))
    adjacency, edge_rows = sparsify_adjacency_top_k(
        adjacency=adjacency, edge_rows=edge_rows,
        feat_cols=feat_cols, top_k_per_node=int(args.top_k_per_node),
    )
    ...
    if len(edges_df) > 0:
        edges_df = edges_df.sort_values(
            ["kept", "mic", "overlap"], ascending=[False, False, False], kind="mergesort"
        )
    ...
    metadata = {
        ...
        "mean_nonzero_mic": (
            float(adjacency[adjacency > 0].mean()) if np.any(adjacency > 0) else 0.0
        ),
