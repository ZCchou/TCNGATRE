...
RUN_PREFIX = "tcngatre_h6_conservative"
...
def resolve_config(argv=None, description="Run H6_Conservative hparam variant.") -> TCNGATREConfig:
    args = _parse_dataset(argv, description)
    os.environ.setdefault(
        "UAV_TCNGATRE_RUN_ROOT",
        str(MODEL_ROOT / "runs" / f"{RUN_PREFIX}_{args.dataset}"),
    )
    # H6_Conservative: stronger regularization, slower learning rate
    os.environ.setdefault("UAV_TCNGATRE_LR", "1e-4")
    os.environ.setdefault("UAV_TCNGATRE_DROPOUT", "0.30")
    os.environ.setdefault("UAV_TCNGATRE_WEIGHT_DECAY", "5e-4")
    os.environ.setdefault("UAV_TCNGATRE_EARLY_STOP_PATIENCE", "10")
