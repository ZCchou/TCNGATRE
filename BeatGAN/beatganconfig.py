...
from data.alfa_shared import (  # noqa: E402
    MANIFEST_NAME,
...
@dataclass
class BeatGANRunConfig:
    dataset_name: str = _env_str("UAV_BEATGAN_DATASET", "alfa")
    data_root: Path | None = None
    split_info_path: Path | None = None
    labels_root: Path | None = None
    run_root: Path | None = None

    window_size: int = _env_int("UAV_BEATGAN_WINDOW_SIZE", 64)
    stride_train: int = _env_int("UAV_BEATGAN_STRIDE_TRAIN", 64)
    stride_val: int = _env_int("UAV_BEATGAN_STRIDE_VAL", 64)
    stride_test: int = _env_int("UAV_BEATGAN_STRIDE_TEST", 64)
    trim_leading_sec: float | None = None
    use_replication_padding: bool = _env_bool("UAV_BEATGAN_USE_REPLICATION_PADDING", True)
    context_mode: str = _env_str("UAV_BEATGAN_CONTEXT_MODE", "window")
    num_features: int = _env_int("UAV_BEATGAN_NUM_FEATURES", 0)
    failure_label_time_offset_sec: float | None = None

    latent_dim: int = _env_int("UAV_BEATGAN_LATENT_DIM", 48)
    base_channels: int = _env_int("UAV_BEATGAN_BASE_CHANNELS", 32)
    lambda_adv: float = _env_float("UAV_BEATGAN_LAMBDA_ADV", 1.0)
    score_mode: str = _env_str("UAV_BEATGAN_SCORE_MODE", "combined")

    batch_size: int = _env_int("UAV_BEATGAN_BATCH_SIZE", 128)
    num_epochs: int = _env_int("UAV_BEATGAN_NUM_EPOCHS", 30)
    lr: float = _env_float("UAV_BEATGAN_LR", 1e-4)
    beta1: float = _env_float("UAV_BEATGAN_BETA1", 0.5)
    beta2: float = _env_float("UAV_BEATGAN_BETA2", 0.999)
    early_stop_patience: int = _env_int("UAV_BEATGAN_EARLY_STOP_PATIENCE", 8)
    early_stop_min_delta: float = _env_float("UAV_BEATGAN_EARLY_STOP_MIN_DELTA", 1e-4)
    num_workers: int = _env_int("UAV_BEATGAN_NUM_WORKERS", 0)
    seed: int = _env_int("UAV_BEATGAN_SEED", 42)
    device: str = _env_str("UAV_BEATGAN_DEVICE", "auto")

    threshold_smooth_alpha: float = _env_float("UAV_BEATGAN_THRESHOLD_SMOOTH_ALPHA", 0.20)
    static_threshold_p: int = _env_int("UAV_BEATGAN_STATIC_THRESHOLD_P", 1000)
    static_threshold_label_col: str = _env_str("UAV_BEATGAN_STATIC_THRESHOLD_LABEL_COL", "label_any")
    dynamic_threshold_history: int = _env_int("UAV_BEATGAN_DYNAMIC_THRESHOLD_HISTORY", 128)
    dynamic_threshold_z_values: tuple[int, ...] = _env_int_list(
        "UAV_BEATGAN_DYNAMIC_THRESHOLD_Z_VALUES",
        (2, 3, 4, 5, 6, 7, 8, 9, 10),
    )
    dynamic_threshold_warmup_pred: int = _env_int("UAV_BEATGAN_DYNAMIC_THRESHOLD_WARMUP_PRED", 0)

    use_ema: bool = _env_bool("UAV_BEATGAN_USE_EMA", False)
    ema_alpha: float = _env_float("UAV_BEATGAN_EMA_ALPHA", 0.20)
    save_per_dim_error: bool = _env_bool("UAV_BEATGAN_SAVE_PER_DIM_ERROR", True)

    infer_output_name: str = _env_str("UAV_BEATGAN_INFER_OUTPUT_NAME", "infer_beatgan_failure")
    infer_split: str = _env_str("UAV_BEATGAN_INFER_SPLIT", "failure")
    plot_scores: bool = _env_bool("UAV_BEATGAN_PLOT_SCORES", True)
    plot_compare_timelines: bool = _env_bool("UAV_BEATGAN_PLOT_COMPARE_TIMELINES", True)
    plot_max_flights: int = _env_int("UAV_BEATGAN_PLOT_MAX_FLIGHTS", 0)
    ...
    def __post_init__(self):
        ...
        env_data_root = os.getenv("UAV_BEATGAN_DATA_ROOT")
        ...
        env_split_info_path = os.getenv("UAV_BEATGAN_SPLIT_INFO_PATH")
        ...
        env_labels_root = os.getenv("UAV_BEATGAN_LABELS_ROOT")
        if env_labels_root:
        ...
        env_run_root = os.getenv("UAV_BEATGAN_RUN_ROOT")
        self.run_root = (
            Path(env_run_root)
            if env_run_root
            else (PORTABLE_ROOT / "BeatGAN" / "runs" / f"beatgan_{self.dataset_name}" if self.run_root is None else Path(self.run_root))
        )

        env_trim = os.getenv("UAV_BEATGAN_TRIM_LEADING_SEC")
        ...
        env_offset = os.getenv("UAV_BEATGAN_FAILURE_LABEL_TIME_OFFSET_SEC")
        ...
        env_norm_stats_path = os.getenv("UAV_BEATGAN_NORMALIZATION_STATS_PATH")
        ...
        self.base_channels = max(int(self.base_channels), 4)
        ...
        if self.score_mode not in {"rec", "combined"}:
