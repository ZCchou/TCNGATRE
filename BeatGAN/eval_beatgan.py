...
from data.usad_window_dataset import add_anomaly_background, attach_window_labels, load_failure_labels  # noqa: E402
from utils.global_threshold import save_global_threshold  # noqa: E402
...
def parse_args(argv: list[str] | None = None):
    parser = argparse.ArgumentParser(description="Evaluate BeatGAN on a bundled dataset.")
...
def plot_scores_by_flight(
    ...
    for flight in flights:
        ...
        score_col = "scores_smooth" if "scores_smooth" in group.columns else "total_score"
        ...
        ax.set_title(f"{flight} | BeatGAN {score_col}")
        ax.set_xlabel("t (sec)")
        ax.set_ylabel("score")
        ax.grid(True, alpha=0.25)
        ax.legend(loc="best")
        fig.tight_layout()
        fig.savefig(out_dir / f"{flight}__beatgan_total_score_threshold.png", dpi=140)
        plt.close(fig)
...
def main(argv: list[str] | None = None):
    ...
    print(f"[OK] BeatGAN evaluation saved to: {analysis_root}")
