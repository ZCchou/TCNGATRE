...
def parse_args(argv: list[str] | None = None):
    parser = argparse.ArgumentParser(description="Evaluate Recurrent AE on a bundled dataset.")
...
def plot_scores_by_flight(
    ...
    for flight in flights:
        ...
        ax.set_title(f"{flight} | {title} {score_col}")
        ax.set_xlabel("t (sec)")
        ax.set_ylabel("score")
        ax.grid(True, alpha=0.25)
        ax.legend(loc="best")
        fig.tight_layout()
        fig.savefig(out_dir / f"{flight}__recurrent_ae_total_score_threshold.png", dpi=140)
        plt.close(fig)
...
def main(argv: list[str] | None = None):
    ...
    print(f"[OK] {model_label(cfg)} evaluation saved to: {analysis_root}")
