...
from revision_experiments.baselines.m2ad_adapter import _parameters as m2ad_parameters
...
def test_m2ad_uses_tcngatre_input_sampling_protocol() -> None:
    expected = {
        "alfa": {"window": 128, "train_stride": 16, "score_stride": 16, "batch_size": 128},
        "gpsdata": {"window": 128, "train_stride": 4, "score_stride": 4, "batch_size": 32},
        "simulate": {"window": 128, "train_stride": 4, "score_stride": 4, "batch_size": 128},
    }
    for dataset, values in expected.items():
        cfg = make_config("ex09", dataset, "m2ad", 0)
        params = m2ad_parameters(cfg, channels=12)
        for key, value in values.items():
            assert params[key] == value
        assert params["tcngatre_reference_horizon"] == 4
        assert params["forecast_horizon"] == 1
        assert params["forecast_horizon_policy"] == "method_native_one_step"
