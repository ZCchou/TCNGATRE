...
MODEL_SPECS: dict[str, ModelSpec] = {
    ...
    "Recurrent_AE": ModelSpec(
        key="Recurrent_AE",
        display_name="Recurrent AE",
        model_dir="Recurrent_AE",
        run_prefix="recurrent_ae",
    ),
    "TranAD": ModelSpec(
        key="TranAD",
        display_name="TranAD",
    ...
    "OmniAnomaly": ModelSpec(
        key="OmniAnomaly",
        display_name="OmniAnomaly",
        model_dir="OmniAnomaly",
        run_prefix="omni_anomaly",
    ),
    "BeatGAN": ModelSpec(
        key="BeatGAN",
        display_name="BeatGAN",
        model_dir="BeatGAN",
        run_prefix="beatgan",
    ),
    "TCNGATRE": ModelSpec(
        key="TCNGATRE",
        display_name="TCNGATRE",
        model_dir="TCNGATRE",
        run_prefix="tcngatre",
    ),
}
...
def parse_args(argv: list[str] | None = None):
    parser = argparse.ArgumentParser(
        description="Summarize anomaly-detection metrics for all bundled models and datasets."
    )
    parser.add_argument(
        "--models",
        nargs="+",
    ...
    parser.add_argument(
        ...
        help="Datasets to include in the report.",
    )
    parser.add_argument(
        "--label-cols",
        nargs="+",
        choices=LABEL_COLS,
        default=LABEL_COLS,
        help="Label columns to include. label_any is used as the main paper-style table.",
    )
    parser.add_argument(
        "--out-dir",
        default="statistic",
        help="Output directory. Relative paths are resolved from the bundle root.",
    )
...
def run_root_for(spec: ModelSpec, dataset: str) -> Path:
    return BUNDLE_ROOT / spec.model_dir / "runs" / f"{spec.run_prefix}_{dataset}"
...
def candidate_score_threshold_dirs(run_root: Path) -> list[Path]:
    ...
    def rank(path: Path) -> tuple[int, int, int, float]:
        ...
        threshold = int(
            ...
            or
            (path / "single_global_threshold_total_score.json").exists()
...
def threshold_methods_for(rows: list[dict[str, Any]]) -> list[str]:
    ...
    ordered = [method for method in THRESHOLD_METHODS if method in seen or len(seen) == 0]
            ...
            for method in methods:
                ...
                table_rows.append(
                    [
                        ...
                        format_macro(
                            None if row is None else row.get("average_precision_mean"),
                            None if row is None else row.get("average_precision_std"),
                        ),
                        format_macro(
                            None if row is None else row.get("accuracy_mean"),
                            None if row is None else row.get("accuracy_std"),
                        ),
                        format_macro(
                            None if row is None else row.get("precision_mean"),
                            None if row is None else row.get("precision_std"),
                        ),
                        format_macro(
                            None if row is None else row.get("recall_mean"),
                            None if row is None else row.get("recall_std"),
                        ),
...
def build_markdown(
    ...
    lines = [
        "# All Model-Dataset Anomaly Detection Metrics",
        "",
        f"Generated at: `{generated}`",
        "",
        "All source paths below are relative to this portable bundle root.",
        "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    micro_df = pd.DataFrame(micro_rows, columns=MICRO_COLUMNS)
    status_df = pd.DataFrame(status_rows, columns=STATUS_COLUMNS)

    macro_csv = out_dir / "all_model_dataset_metrics_macro.csv"
    micro_csv = out_dir / "all_model_dataset_metrics_micro.csv"
    status_csv = out_dir / "all_model_dataset_metrics_status.csv"
    markdown_path = out_dir / "all_model_dataset_metrics.md"
