...
def compute_cross_dim_loss(
    ...
    """
    Randomly zero out a subset of sensor channels in x, then compute the
    forecasting loss only on those masked channels.  Forces the model to
    infer masked-sensor futures from graph neighbours, strengthening
    cross-sensor dependencies learned by the graph correction modules.
    """
    num_nodes = x.shape[2]
    device = x.device
...
def main(runtime_cfg: TCNGATREConfig | None = None) -> None:
    ...
    # Compute val normal scores for threshold calibration
    ...
    print(f"[OK] TCNGATRE training complete: {out_dir}")
