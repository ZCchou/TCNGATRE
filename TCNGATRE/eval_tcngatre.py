...
from data.window_labels import add_anomaly_background, attach_window_labels, load_failure_labels  # noqa: E402
...
def plot_scores_by_flight(
    ...
    for flight in flights:
        ...
        ax.set_title(f"{flight} | TCNGATRE {score_col}")
        ax.set_xlabel("t (sec)")
        ax.set_ylabel("score")
        ax.grid(True, alpha=0.25)
        ax.legend(loc="best")
        fig.tight_layout()
        fig.savefig(out_dir / f"{flight}__tcngatre_total_score_threshold.png", dpi=140)
        plt.close(fig)
...
def main(argv: list[str] | None = None):
    ...
    print(f"[OK] TCNGATRE evaluation saved to: {analysis_root}")
