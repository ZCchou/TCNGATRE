...
from tcngatre_train_impl import (
    build_model,
    ensure_dir,
    load_graph,
    save_json,
...
# ---------------------------------------------------------------------------
# flight plot (full visualization, top-20 per dimension)
# ---------------------------------------------------------------------------
...
def plot_flight_outputs(
    ...
    # Rank sensors by mean anomaly score (descending)
...
def main(runtime_cfg: TCNGATREConfig | None = None) -> None:
    ...
    print(f"[OK] TCNGATRE inference saved to: {out_dir}")
