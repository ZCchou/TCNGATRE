...
class MultiHopGraphCorrection(nn.Module):
    ...
    The attention graph (A_fuse) is computed once from h_ctx (recent-window mean),
    then `num_hops` rounds of message passing propagate information across hops.
    Each hop uses the same A_fuse but updates h iteratively, enabling multi-hop
    information flow without extra attention computations.

    Input:
        h_nodes: [B, N, D]  states to update
        h_ctx:   [B, N, D]  context for dynamic attention (recent-window mean)
        a_stat:  [N, N]
        m_mask:  [N, N]
    Output:
        h_out:   [B, N, D]
...
class STGraphTCN(nn.Module):
    ...
    Every `interleave_every` TCNBlocks, a MultiHopGraphCorrection is applied on
    the last-step representation; the corrected state is injected back into the
    sequence, so subsequent TCN blocks see spatially-informed context.

    Cross-dimension masking is handled externally (in the training loop) by
    zeroing out selected sensor channels in x before calling forward.

    Input:
        x:          [B, T, N, F]
        a_stat:     [N, N]
        m_mask:     [N, N]
        node_index: optional [N_sub]
        short_patch: recent steps used as graph attention context
    ...
    def __init__(
        ...
        d_model: int = 64,
        short_kernel: int = 5,
        nhead: int = 4,        # kept only for API compatibility
        tcn_layers: int = 5,
        dropout: float = 0.1,
        eta: float = 1.0,
        beta: float = 0.5,
        ...
        # graph correction after every interleave_every blocks; always at the final block
    ...
    def forward(
        ...
        for block_idx, tcn_block in enumerate(self.tcn_block_list):
            h_flat = tcn_block(h_flat)  # [B*N, T, D]

            if block_idx in self._correction_positions:
                h_last = h_flat[:, -1, :].view(batch, num_nodes, self.d_model)            # [B, N, D]
                ...
                # Inject corrected last step back; subsequent TCN blocks see spatial context
                h_flat = torch.cat(
                    [h_flat[:, :-1, :], h_corrected.view(batch * num_nodes, 1, self.d_model)],
                    dim=1,
                )

        h_final = h_flat[:, -1, :].view(batch, num_nodes, self.d_model)  # [B, N, D]
        y = self.pred_head(h_final)                                        # [B, N, H*F_out]
