...
MODEL_DISPLAY = {
    ...
    "TCNGATRE_NoCrossD": "No Cross-Dim Loss",
    "TCNGATRE_StaticG": "Static Graph Only",
    "TCNGATRE_NoGraph": "No Graph (Pure TCN)",
    "TCNGATRE_SingleHop": "Single-Hop GNN",
}
RUN_PREFIX = {
...
def parse_args(argv: list[str] | None = None):
    parser = argparse.ArgumentParser(
        description="Summarize TCNGATRE ablation metrics across all variants and datasets."
    )
    parser.add_argument(
        "--models",
        nargs="+",
    ...
    parser.add_argument(
        "--out-dir",
        default="statistic",
        help="Output directory. Relative paths resolve from ablation root.",
    )
...
def run_root_for(spec: ModelSpec, dataset: str) -> Path:
    return ABLATION_ROOT / spec.model_dir / "runs" / f"{spec.run_prefix}_{dataset}"
...
def build_markdown(
    ...
    lines = [
        "# TCNGATRE Ablation Study Metrics",
        "",
        f"Generated at: `{generated}`",
        "",
        "All source paths below are relative to the ablation directory.",
        "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    macro_csv = out_dir / "ablation_metrics_macro.csv"
    micro_csv = out_dir / "ablation_metrics_micro.csv"
    status_csv = out_dir / "ablation_metrics_status.csv"
    markdown_path = out_dir / "ablation_metrics.md"
