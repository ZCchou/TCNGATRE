...
def resolve_config(argv=None, description="Run TCNGATRE_NoGraph ablation.") -> TCNGATREConfig:
    args = _parse_dataset(argv, description)
    os.environ.setdefault(
        "UAV_TCNGATRE_RUN_ROOT",
        str(MODEL_ROOT / "runs" / f"{RUN_PREFIX}_{args.dataset}"),
    )
    # No cross-dim loss — no graph correction means no cross-sensor dependency in model
    os.environ.setdefault("UAV_TCNGATRE_CROSS_DIM_LOSS_ENABLED", "0")
    return TCNGATREConfig(dataset_name=args.dataset)


def ensure_graph_ready(cfg: TCNGATREConfig) -> None:
    # Graph is built to obtain node_names; the model itself ignores a_stat and m_mask.
