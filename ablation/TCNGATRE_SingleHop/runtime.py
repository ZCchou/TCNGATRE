...
def resolve_config(argv=None, description="Run TCNGATRE_SingleHop ablation.") -> TCNGATREConfig:
    args = _parse_dataset(argv, description)
    os.environ.setdefault(
        "UAV_TCNGATRE_RUN_ROOT",
        str(MODEL_ROOT / "runs" / f"{RUN_PREFIX}_{args.dataset}"),
    )
    # Single-hop message passing (original default is 2)
    os.environ.setdefault("UAV_TCNGATRE_GRAPH_NUM_HOPS", "1")
