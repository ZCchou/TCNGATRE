...
# Identical to TCNGATRE_Full/train_impl.py except cross_dim_loss is permanently disabled.
# The cfg.cross_dim_loss_enabled flag is also set to False via env in runtime.py,
# but run_epoch here hard-codes the skip to make the intent explicit.
...
def save_loss_curve(history: Dict[str, List[float]], path: Path) -> None:
    ...
    ax.set_title("TCNGATRE_NoCrossD Training Loss Curve")
    ax.set_xlabel("epoch")
    ax.set_ylabel("loss")
...
def make_dataloaders(
    ...
    dl_kwargs = dict(
        ...
        collate_fn=collate_stgtcn_windows, pin_memory=torch.cuda.is_available(),
    )
...
def fit_normalization_stats(cfg: TCNGATREConfig, node_names: Sequence[str]) -> dict:
    ...
    for _, csv_path in train_paths.items():
        ...
        if x_raw.size <= 0:
            continue
        x_nodes = project_node_features(x_raw=x_raw, feature_names_all=list(cols), node_names=list(node_names))
...
def run_epoch(
    ...
    for batch in tqdm(dl, desc=desc, unit="batch", dynamic_ncols=True, leave=False):
        ...
        # Cross-dim loss is permanently disabled in NoCrossD variant
...
def main(runtime_cfg: TCNGATREConfig | None = None) -> None:
    ...
    for epoch in epoch_iter:
        ...
        history["train_cross_dim_loss"].append(0.0)
        ...
        print(f"[INFO] epoch={epoch+1} train_loss={train_m['loss']:.6f} val_loss={val_m['loss']:.6f} best_val={best_val:.6f}")
    ...
    payload = {
        ...
        "dataset": {"train_windows": len(train_ds), "val_windows": len(val_ds),
