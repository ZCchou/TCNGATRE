# -*- coding: utf-8 -*-
# A3 StaticG variant: DynamicGraphAttention.forward() returns the normalized static MIC graph
# instead of the learned QK attention, so the model uses purely static graph topology.
...
class DynamicGraphAttention(nn.Module):
    """Static-only variant: ignores Q/K attention, returns normalized MIC static graph."""

    def __init__(self, d_model: int, eta: float = 1.0):
        super().__init__()
        # Keep Q/K params so checkpoints remain compatible, but they are not used in forward.
    ...
    def forward(
        ...
        # Build normalized static graph (same logic as MultiHopGraphCorrection._build_static_graph)
        ...
        a_static = a_static / a_static.sum(dim=-1, keepdim=True).clamp_min(1e-8)
        # Expand to batch dimension
        return a_static.unsqueeze(0).expand(batch, -1, -1)
...
class MultiHopGraphCorrection(nn.Module):
    ...
    def forward(
        ...
        # a_dyn already returns static graph from DynamicGraphAttention
        a_dyn = self.dyn(h_ctx, a_stat, m_mask)
        a_static = self._build_static_graph(a_stat, m_mask)
        # With beta=0 (set via env in runtime) and a_dyn == a_static, a_fuse == a_static
...
class STGraphTCN(nn.Module):
    def __init__(
        ...
        self.graph_corrections = nn.ModuleList([
            MultiHopGraphCorrection(
                d_model=self.d_model, eta=eta, beta=beta, dropout=dropout,
                topk=graph_topk, gate_init=graph_gate_init, num_hops=num_hops,
            )
    ...
    def _select_graph(self, a_stat, m_mask, node_index, num_nodes):
        ...
        if a_stat.shape != m_mask.shape:
            raise ValueError(f"a_stat and m_mask shape mismatch: {tuple(a_stat.shape)} vs {tuple(m_mask.shape)}")
        ...
        if a_stat.shape != (num_nodes, num_nodes):
            raise ValueError(f"graph shape must match x num_nodes {num_nodes}, got {tuple(a_stat.shape)}")
    ...
    def forward(self, x, a_stat, m_mask, node_index=None, short_patch=5):
        ...
        a_stat, m_mask = self._select_graph(
            a_stat.to(device=x.device), m_mask.to(device=x.device), node_index, num_nodes
        )
        ...
        for block_idx, tcn_block in enumerate(self.tcn_block_list):
            ...
            if block_idx in self._correction_positions:
                ...
                h_corrected, graph_aux = self.graph_corrections[correction_idx](h_last, h_ctx, a_stat, m_mask)
                correction_idx += 1
                last_aux = graph_aux
                h_flat = torch.cat(
                    [h_flat[:, :-1, :], h_corrected.view(batch * num_nodes, 1, self.d_model)], dim=1
                )
