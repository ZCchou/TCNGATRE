...
import numpy as np
import pandas as pd
import summarize_ablation as summarize_results
...
MODEL_DISPLAY = {
    "TCNGATRE_Full": "TCNGATRE (Full)",
    "TCNGATRE_NoCrossD": "No Cross-Dim",
    "TCNGATRE_StaticG": "Static Graph",
    "TCNGATRE_NoGraph": "No Graph",
    "TCNGATRE_SingleHop": "Single-Hop",
}
MODEL_COLORS = {
    "TCNGATRE_Full": "#3A86FF",
    "TCNGATRE_NoCrossD": "#FF6B6B",
    "TCNGATRE_StaticG": "#FFA500",
    "TCNGATRE_NoGraph": "#2D6A4F",
    "TCNGATRE_SingleHop": "#7B2CBF",
}
RUN_PREFIX = {
    "TCNGATRE_Full": "tcngatre_full",
    "TCNGATRE_NoCrossD": "tcngatre_nocrossd",
    "TCNGATRE_StaticG": "tcngatre_staticg",
    "TCNGATRE_NoGraph": "tcngatre_nograph",
    "TCNGATRE_SingleHop": "tcngatre_singlehop",
}
...
def parse_args(argv: list[str] | None = None):
    parser = argparse.ArgumentParser(
        description="Plot grouped bar charts from TCNGATRE ablation metric summaries."
    )
    parser.add_argument("--models", nargs="+", default=MODEL_ORDER)
    parser.add_argument(
        "--stat-dir",
        default="statistic",
        help="Directory containing ablation_metrics_*.csv. Relative paths resolve from ablation root.",
    )
    parser.add_argument(
        "--out-dir",
    ...
    parser.add_argument("--averages", nargs="+", choices=AVERAGE_KINDS, default=AVERAGE_KINDS)
    parser.add_argument("--datasets", nargs="+", default=DATASET_ORDER)
    parser.add_argument("--label-cols", nargs="+", default=LABEL_COLS)
    parser.add_argument(
        "--threshold-methods",
        nargs="+",
        default=["static_f1_oracle", "static_val_sigma3", "dynamic_history", "spot"],
    )
    parser.add_argument("--metrics", nargs="+", default=METRIC_ORDER)
    parser.add_argument("--dpi", type=int, default=180)
    parser.add_argument("--no-error-bars", action="store_true")
    parser.add_argument("--no-threshold-plots", action="store_true")
    parser.add_argument("--max-flights-per-run", type=int, default=0)
    parser.add_argument("--threshold-downsample", type=int, default=1)
    parser.add_argument("--skip-auto-summarize", action="store_true")
...
def relative_to_ablation(path: Path) -> str:
    try:
        return path.resolve().relative_to(ABLATION_ROOT).as_posix()
    except ValueError:
        return str(path.resolve())


def status_summary_lines(stat_dir: Path) -> list[str]:
    status_path = stat_dir / "ablation_metrics_status.csv"
    ...
    if df.empty or "status" not in df.columns:
        return []
    counts = df["status"].astype(str).value_counts(dropna=False).sort_index()
...
def safe_write_text(path: Path, text: str) -> bool:
    try:
        ...
        print(f"[WARN] could not write {relative_to_ablation(path)}: {type(exc).__name__}: {exc}")
        return False
...
def draw_grouped_metric_bars(
    ...
    if not available_metrics:
        return None
    model_keys = [model for model in MODEL_ORDER if model in set(group["model_key"].astype(str))]
    model_keys += [model for model in group["model_key"].astype(str).tolist() if model not in model_keys]
    if not model_keys:
        return None
    ...
    for model in model_keys:
        ...
        values_by_model[model] = [float(metric_value(row, m, average_kind)) for m in available_metrics]
        std_by_model[model] = [
            float(metric_std(row, m)) if average_kind == "macro" else math.nan
    ...
    ax.set_title(
        f"TCNGATRE Ablation | {title_average} | dataset={dataset} | threshold={title_method} | label={title_label}",
        fontsize=12,
        fontweight="bold",
        pad=14,
    )
    ...
    try:
        ...
        print(f"[WARN] could not write {relative_to_ablation(output_path)}: {type(exc).__name__}: {exc}")
        plt.close(fig)
        return None
    plt.close(fig)
    return PlotSpec(
...
def run_root_for(model_key: str, dataset: str) -> Path:
    prefix = RUN_PREFIX.get(model_key, model_key.lower())
    return ABLATION_ROOT / model_key / "runs" / f"{prefix}_{dataset}"
...
def sequence_scores_path_for(model_key: str, dataset: str) -> Path | None:
    analysis_dir = best_analysis_dir(run_root_for(model_key, dataset))
...
def draw_threshold_timeseries(
    ...
    threshold_static_val = (
        ...
        if threshold_static_val_col is not None else np.asarray([], dtype=float)
    )
    ...
    threshold_spot = (
        ...
        if threshold_spot_col is not None else np.asarray([], dtype=float)
    )
    ...
    ax.plot(x, threshold_static, color="#F4A261", linewidth=1.35, linestyle="--",
            label=f"static ({threshold_static_col})")
    if threshold_static_val_col is not None:
        ax.plot(x, threshold_static_val, color="#8D99AE", linewidth=1.2,
                linestyle=(0, (4, 2)), alpha=0.95, label=f"val 3sigma ({threshold_static_val_col})")
    ax.plot(x, threshold_dynamic, color="#2A9D8F", linewidth=1.25, linestyle="-.",
            alpha=0.92, label=f"dynamic ({threshold_dynamic_col})")
    if threshold_spot_col is not None:
        ax.plot(x, threshold_spot, color="#6A4C93", linewidth=1.15, linestyle=":",
                alpha=0.95, label=f"SPOT ({threshold_spot_col})")
    ...
    ax.spines["right"].set_visible(False)
    ax.set_title(
        f"{MODEL_DISPLAY.get(model_key, model_key)} | dataset={dataset} | flight={flight} | label={label_col}",
        fontsize=12, fontweight="bold", pad=12,
    )
    ax.legend(loc="upper right", frameon=True, framealpha=0.92, fontsize=9)
    ax.text(0.0, -0.15,
            "Red background = ground-truth anomaly. Markers show anomaly predictions per threshold method.",
...
def refresh_statistics_if_needed(args) -> None:
    ...
    try:
        ...
        print(f"[WARN] auto summarize failed; using existing CSV if available: {type(exc).__name__}: {exc}")
...
def plot_all(args) -> tuple[list[PlotSpec], list[ThresholdPlotSpec]]:
    ...
    csv_by_average = {
        "macro": stat_dir / "ablation_metrics_macro.csv",
        "micro": stat_dir / "ablation_metrics_micro.csv",
    }
    data_by_average = {
        ...
        for dataset in datasets:
            for threshold_method in threshold_methods:
                for label_col in label_cols:
                    ...
                    filename = f"{average_kind}__{dataset}__{threshold_method}__{label_col}.png"
...
def write_plot_index(
    ...
    try:
        ...
        print(f"[WARN] could not create plot directory: {type(exc).__name__}: {exc}")
        return
    lines: list[str] = [
        "# TCNGATRE Ablation Metric Plots",
        "",
        "## Sources",
        "",
    ]
    for average, path in csv_by_average.items():
        ...
        lines.append(f"- `{average}`: `{relative_to_ablation(path)}` (`{rows}` rows)")
    lines.extend(["", "## Generated Metric PNGs", ""])
    if not created:
        lines.append("No metric plots generated. Run ablation pipeline first.")
    else:
        for spec in created:
            rel_path = relative_to_ablation(spec.output_path)
            lines.append(
    ...
    if not threshold_created:
        lines.append("No threshold plots generated.")
    else:
...
def main(argv: list[str] | None = None) -> int:
    ...
    out_dir = resolve_path(args.out_dir)
    stat_dir = resolve_path(args.stat_dir)
    print(f"[OK] plot_dir={relative_to_ablation(out_dir)}")
    ...
    for spec in created:
        print(
            f"[METRIC PNG] {spec.average_kind} {spec.dataset} {spec.threshold_method} "
            f"{spec.label_col} rows={spec.rows} path={relative_to_ablation(spec.output_path)}"
        )
    status_lines = status_summary_lines(stat_dir)
    if status_lines:
        print(f"[STATUS] {'; '.join(status_lines)}")
    print(f"[OK] index={relative_to_ablation(out_dir / 'plot_index.md')}")
    return 0
