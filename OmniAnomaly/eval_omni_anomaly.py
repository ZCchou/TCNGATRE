...
from utils.metrics import summarize_per_flight, summarize_threshold_metrics
...
def plot_scores_by_flight(
    ...
    for flight in flights:
        ...
        ax.set_title(f"{flight} | OmniAnomaly {score_col}")
        ax.set_xlabel("t (sec)")
        ax.set_ylabel("score")
        ax.grid(True, alpha=0.25)
        ax.legend(loc="best")
        fig.tight_layout()
        fig.savefig(out_dir / f"{flight}__omni_anomaly_total_score_threshold.png", dpi=140)
        plt.close(fig)
...
def main(argv: list[str] | None = None):
    ...
    if not failure_scores_path.exists():
        legacy_path = infer_root / "all_failure_future_forecast_residual.csv"
        if legacy_path.exists():
            failure_scores_path = legacy_path
    if not failure_scores_path.exists():
    ...
    print(f"[OK] OmniAnomaly evaluation saved to: {analysis_root}")
